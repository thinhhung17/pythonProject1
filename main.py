...
def _one_layer(input_,output_size,activation_fn=tf.nn.relu,stddev=5.0, name='linear'):
  with tf.variable_scope(name):
    ...
    w=tf.get_variable('Matrix',[shape[1],output_size],tf.float64,tf.random_normal_initializer(stddev=stddev/np.sqrt(shape[1]+output_size)))
...
def _batch_norm(x,name):
  with tf.variable_scope(name):
    ...
    beta=tf.get_variable('beta',params_shape,tf.float64,initializer=tf.random_normal_initializer(0.0,stddev=0.1))
    gamma=tf.get_variable('gamma',params_shape,tf.float64,initializer=tf.random_uniform_initializer(0.1,0.5))
    moving_mean=tf.get_variable('moving_mean',params_shape,tf.float64,initializer=tf.constant_initializer(0.0),trainable=False)
    moving_variance=tf.get_variable('moving_variance',params_shape,tf.float64,initializer=tf.constant_initializer(1.0),trainable=False)
...
with tf.Session() as sess:
  ...
  Z0=tf.Variable(tf.random_uniform([1,d],minval=-0.1,maxval=0.1,dtype=tf.float64),name='Z0')
  Gamma0=tf.Variable(tf.random_uniform([d,d],minval=-1,maxval=1,dtype=tf.float64),name='Gamma0')
  A0=tf.Variable(tf.random_uniform([1,d],minval=-0.1,maxval=0.1,dtype=tf.float64),name='A0')
